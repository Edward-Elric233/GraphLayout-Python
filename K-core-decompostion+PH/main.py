...
nodes = []
data_nodes = []
(nodes,data_nodes) = dataProcess()
PH()


# 读入文件
time_start = time.time()
with open('test_data.edges', 'r') as fp, open('input.csv', 'w') as wp:
    wp.write('source,target,length,relation,\n')
    for line in fp:
        tmp = list(line.split())
        u = int(tmp[0]) + 1
        v = int(tmp[1]) + 1
...
with open('output.csv', 'w') as fp:
    fp.write("id,x,y,k,name,category,\n")
    for i in range(1, len(data)):
        n = data[i]
        fp.write("%d,%f,%f,%d,%s,%s,\n"%(i,n.x,n.y,n.k,nodes[i-1],data_nodes[nodes[i-1]]['type']))
